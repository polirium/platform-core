...
const baseTemplate_1 = __importDefault(require("./templates/baseTemplate"));
const dataGet_1 = __importDefault(require("@/utils/dataGet"));
const notifications_1 = require("@/notifications");
const props_1 = require("@/alpine/magic/props");
...
exports.default = (initOptions) => ({
    ...
    initWireModel() {
        ...
        if (this.hasWireModel && this.config.multiselect) {
            this.$watch('selectedOptions', (options, oldOptions) => {
                if (this.mustSyncWireModel()) {
                    this.wireModel = options.map((option) => option.value);
                }
                if (JSON.stringify(options) !== JSON.stringify(oldOptions)) {
                    this.syncSelectedOptions();
                }
            });
            this.$watch('wireModel', (options) => {
        ...
        if (this.hasWireModel && !this.config.multiselect) {
            this.$watch('selected', (option, oldOption) => {
    ...
    shouldSyncProps(mutations = []) {
        return mutations.some(mutation => {
            return mutation.attributeName === 'x-props';
        });
    },
    syncProps(mutations = []) {
        if (mutations.length && !this.shouldSyncProps(mutations))
            return;
    ...
    syncSlotOptions() {
        ...
        const options = Array.from(elements).flatMap(element => {
            ...
            if (!base64)
                return [];
            const option = window.Alpine.evaluate(this, base64);
    ...
    makeRequest(params = {}) {
        ...
        const parameters = Object.assign(params, window.Alpine.raw(this.asyncData.params), ...Array.from(url.searchParams).map(([key, value]) => ({ [key]: value })));
        url.search = '';
        if (method === 'GET') {
    ...
    fetchOptions() {
        ...
        fetch(this.makeRequest({ search: this.search }))
            ...
            .then((json) => {
            const rawOptions = (0, dataGet_1.default)(json, this.asyncData.optionsPath);
            if (!Array.isArray(rawOptions))
                return;
            this.setOptions(rawOptions.map(rawOption => this.mapOption(rawOption)));
            this.$nextTick(() => this.initRenderObserver());
        }).catch((message) => {
    ...
    fillSelectedFromInputValue() {
        ...
        try {
            this.selectedOptions = (0, helpers_1.jsonParse)(inputValue, []).map(value => {
    ...
    getValue() {
        try {
            const values = this.hasWireModel
                ? this.wireModel
                : (0, helpers_1.jsonParse)(this.$refs.input.value);
            if (!values)
                return [];
            return [values].flat();
        }
    ...
    getSelectedDisplayText() {
        ...
        if (this.selected.template) {
            ...
            const template = templates_1.templates[this.selected.template](config);
    ...
    getPlaceholder() {
        if (this.config.multiselect && this.selectedOptions.length > 0)
            return '';
    ...
    select(option) {
        if (this.config.readonly || option.disabled || option.readonly)
            return;
        this.search = '';
        if (this.config.multiselect) {
    ...
    unSelect(option) {
        if (this.config.readonly || !this.config.clearable)
            return;
