...
@php
    $fileValue = $value;
    $isFullUrl = $fileValue && (str_starts_with($fileValue, 'http://') || str_starts_with($fileValue, 'https://'));
    $imgSrc = $fileValue ? ($isFullUrl ? $fileValue : asset($fileValue)) : null;

    // Get extension (remove query string first)
    $urlPath = $fileValue ? (parse_url($fileValue, PHP_URL_PATH) ?? $fileValue) : '';
    $fileExt = $urlPath ? strtolower(pathinfo($urlPath, PATHINFO_EXTENSION)) : '';
    $imageExts = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'ico', 'svg', 'bmp'];
    $isImage = $fileValue && (in_array($fileExt, $imageExts) || $isFullUrl);
