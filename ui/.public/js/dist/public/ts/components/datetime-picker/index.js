...
const makeTimes_1 = require("./makeTimes");
...
exports.default = (options) => ({
    ...
    syncCalendar() {
        if (!this.input?.getYear || !this.input?.getMonth)
            return;
    ...
    fillPickerDates() {
        const date = (0, date_1.date)(`${this.year}-${this.month + 1}`, this.localTimezone);
    ...
    fillTimes() {
        if (this.times.length > 0)
            return;
        const times = (0, makeTimes_1.makeTimes)({
    ...
    isSelected(date) {
        if (!this.model)
            return false;
        const model = (0, date_1.date)(this.model, this.timezone, this.parseFormat);
        const compare = (0, date_1.date)(date.date, this.userTimezone);
    ...
    onSearchTime(search) {
        ...
        this.searchTime = (0, masker_1.applyMask)(mask, search) ?? '';
        this.filteredTimes = this.filterTimes(this.times.filter(time => time.label.includes(this.searchTime ?? '')));
    ...
    makeSearchTimes(search) {
        ...
        times.push({
            value: (0, time_1.convertStandardTimeToMilitary)(`${search} AM`),
            label: `${search} AM`
        });
        times.push({
            value: (0, time_1.convertStandardTimeToMilitary)(`${search} PM`),
            label: `${search} PM`
        });
        return this.filterTimes(times);
    },
