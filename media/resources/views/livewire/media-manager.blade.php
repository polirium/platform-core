        ...
        <div class="flex-grow-1" :class="showSidebar ? 'me-3' : ''">
            ...
            @if(count($clipboard) > 0)
                <div class="alert alert-warning d-flex justify-content-between align-items-center py-2 mb-3">
                    <span>
                        ...
                        {{ count($clipboard) }} file đang chờ dán
                    </span>
            ...
            @if(count($selectedMedia) > 0)
                <div class="alert alert-primary d-flex justify-content-between align-items-center py-2 mb-3">
                    <span>{{ count($selectedMedia) }} {{ __('item đã chọn') }}</span>
                    <div class="d-flex gap-2">
                        <button type="button" wire:click="cut({{ json_encode($selectedMedia) }})" class="btn btn-sm btn-outline-primary">
                            {{ __('Cắt') }}
                        </button>
                        <button type="button" wire:click="deleteSelected" onclick="return confirm('{{ __('Xác nhận xóa các file đã chọn?') }}')" class="btn btn-sm btn-outline-danger">
                            {{ __('Xóa') }}
                        </button>
                ...
                <div class="media-grid">
                    ...
                    @if($currentFolder)
                        <div class="media-grid-item" wire:click="navigateUp">
                            ...
                            <div class="media-info"><div class="media-name">{{ __('Quay lại') }}</div></div>
                        </div>
                    @endif
                        ...
                        @if(count($folders) === 0 && !$currentFolder)
                            <div class="col-12 w-100">
                                <div class="empty py-5 text-center">
                                    ...
                                    <p class="empty-title text-muted">{{ __('Chưa có file nào') }}</p>
                                    <p class="empty-subtitle text-muted small">{{ __('Click Upload để tải file lên hoặc tạo folder mới.') }}</p>
                                </div>
                            </div>
                        @endif
                    @endforelse
                ...
                <div class="table-responsive">
                    <table class="table table-hover table-vcenter">
                        ...
                        <tbody>
                            ...
                            @empty
                                @if(count($folders) === 0)
                                    <tr><td colspan="5" class="text-center text-muted py-4">{{ __('Chưa có dữ liệu') }}</td></tr>
                                @endif
                            @endforelse
                        </tbody>
                    </table>
