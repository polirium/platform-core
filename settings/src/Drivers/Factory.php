...
namespace Polirium\Core\Settings\Drivers;
...
class Factory
{
    ...
    {
        $this->app['config']['core.settings.config.driver'] = $driver;
    }
    ...
    protected function createDatabaseDriver(array $config): DatabaseDriver
    {
        return new DatabaseDriver(
            ...
            table: $this->app['config']['core.settings.config.table'],
            teamForeignKey: $this->app['config']['core.settings.config.team_foreign_key'] ?? null,
        );
    }
    ...
    {
        return $this->app['config']['core.settings.config.driver'];
    }

    protected function getDriverConfig(string $driver): ?array
    {
        return $this->app['config']["core.settings.config.drivers.{$driver}"];
    }
